...
Expose the Web UI approval endpoint (`/api/approvals/decision`) somewhere reachable (for example via your reverse proxy). Intercessio automatically derives the URL from `IC_LINK` (e.g. `http://dev.otherstuff.studio:4173/`) by appending `/api/approvals/decision`, or you can override it with `NTFY_APPROVAL_ENDPOINT`/`INTERCESSIO_NTFY_APPROVAL_ENDPOINT`. Once set, ntfy notifications include **Approve** / **Reject** buttons that issue POST requests with `{ "approvalId": "...", "approved": true|false }`, so you can resolve requests directly from the notification.
